...
class BottleneckMPAM(nn.Module):
    """Standard bottleneck."""
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        ...
        self.add = shortcut and c1 == c2
        self.mp = MPAM(c1, c2)
        self.mpam = Conv(c2*3, c2, 1)

    def forward(self, x):
        """'forward()' applies the YOLOv5 FPN to input data."""
        y = self.mpam(self.mp(x))
        z = self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
        w = x + y + z
        print("s")
        return w

class C2fMPAM(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList(BottleneckMPAM(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
#组建C2fGhostBottleneck
class C2fGhostBottleneck(nn.Module):
    """CSP Bottleneck with 2 convolutions."""
...
class WeightedPermuteMLP(nn.Module):
    def __init__(self, dim, segment_dim=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., norm_layer=nn.LayerNorm):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #print(S)
        #print(C)
        #print(self.segment_dim)
        h = x.reshape(B, H, W, self.segment_dim, S).permute(0, 3, 2, 1, 4).reshape(B, H, W, self.segment_dim * S)
       # print(h.shape)
        h = self.mlp_h(h).reshape(B, self.segment_dim, W, H, S).permute(0, 3, 2, 1, 4).reshape(B, H, W, C)
        #print(h.shape)
        w = x.reshape(B, H, W, self.segment_dim, S).permute(0, 1, 3, 2, 4).reshape(B, H, W, self.segment_dim * S)
...
class MPAMF(nn.Module):
    ...
    def forward(self, x):
        ...
        w = torch.cat((x0, x1, x2, x3), dim=1)
        q = torch.cat((y0, y1, y2, y3), dim=1)
...
class SELayer(nn.Module):
    def __init__(self, channel,channel_out, reduction=4):
