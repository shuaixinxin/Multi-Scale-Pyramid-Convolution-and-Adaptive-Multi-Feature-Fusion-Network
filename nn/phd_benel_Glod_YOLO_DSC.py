...
class DSC(nn.Module):
    def __init__(self, ch_in, ch_out, k=1, s=1, p=None, g=1, act=True):
        ...
        self.ch_in = ch_in
        self.ch_out = ch_out
        self.depth_conv = nn.Conv2d(ch_in, ch_in, k, s, autopad1(k, p),  groups=ch_in, bias=False)
...
class SimFusion_3in(nn.Module):
    def __init__(self, in_channel_list, out_channels):
        super().__init__()
        self.cv1 = DSC(in_channel_list[0], out_channels, act=nn.ReLU()) if in_channel_list[
                                                                                0] != out_channels else nn.Identity()
        self.cv2 = DSC(in_channel_list[1], out_channels, act=nn.ReLU()) if in_channel_list[
                                                                                1] != out_channels else nn.Identity()
        self.cv3 = DSC(in_channel_list[2], out_channels, act=nn.ReLU()) if in_channel_list[
                                                                                2] != out_channels else nn.Identity()
        self.cv_fuse = DSC(out_channels * 3, out_channels, act=nn.ReLU())
...
class IFM_MPAMFpro(nn.Module):
    def __init__(self, inc, ouc, embed_dim_p=96, fuse_block_num=3) -> None:
        super().__init__()

        self.conv = nn.Sequential(
            MPAMFpro(inc, embed_dim_p),
            *[RepVGGBlock(embed_dim_p, embed_dim_p) for _ in range(fuse_block_num)],
            Conv(embed_dim_p, sum(ouc))
        )
...
class MPAMFpro(nn.Module):
    """Mult-proportion attention mechanism - Fast (SPPF) layer for YOLOv5 by Glenn Jocher."""

    def __init__(self, c1, c2):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv0 = Conv(c1, c_, 1, 1)
        self.cv1 = Conv1X3(c_, c_)
        self.cv2 = Conv3X1(c_, c_)
        self.cv3 = Conv(c_*8, c2, 1, 1)
        self.cv4 = Conv(c1, c1, 3, 1)
    ...
    def forward(self, x):
        ...
        q = torch.cat((y1, y2, y3), dim=1)
        z = torch.cat((w, q, cancha), dim=1)
        s = self.cv3(z)
        return s
#1X3 3X1
class InjectionMultiSum_Auto_poolx(nn.Module):
    def __init__(
            self,
            inp: int,
            oup: int,
        ...
        self.global_act1X3 = Conv(global_inp[self.flag], oup, k=(1, 3), s=1, p=(0, 1), act=False)
        self.global_act3X1 = Conv(global_inp[self.flag], oup, k=(3, 1), s=1, p=(1, 0), act=False)
    ...
    def forward(self, x):
        ...
        global_feat1X3 = self.global_act1X3(gloabl_info)
        global_feat3X1 = self.global_act3X1(gloabl_info)
        if use_pool:
            ...
            global_feat1X3 = avg_pool(global_featglobal_feat1X3, output_size)
            global_feat3X1 = avg_pool(global_featglobal_feat1X3, output_size)

        else:
            sig_act = F.interpolate(self.act(global_act), size=(H, W), mode='bilinear', align_corners=False)
            global_feat = F.interpolate(global_feat, size=(H, W), mode='bilinear', align_corners=False)
            global_feat1X3 = F.interpolate(global_feat1X3, size=(H, W), mode='bilinear', align_corners=False)
            global_feat3X1 = F.interpolate(global_feat3X1, size=(H, W), mode='bilinear', align_corners=False)

        out = local_feat * sig_act + global_feat + global_feat1X3 + global_feat3X1
...
class InjectionMultiSum_Auto_pool(nn.Module):
    def __init__(
        ...
        self.global_inp = global_inp
        self.flag = flag
        self.local_embedding = DSC(inp, oup, 1, act=False)
        self.global_embedding = DSC(global_inp[self.flag], oup, 1, act=False)
        self.global_act = DSC(global_inp[self.flag], oup, 1, act=False)
        self.act = h_sigmoid()

        #self.global_act1X3 = Conv(global_inp[self.flag], oup, k=(1, 3), s=1, p=(0, 1), act=False)
        #self.global_act3X1 = Conv(global_inp[self.flag], oup, k=(3, 1), s=1, p=(1, 0), act=False)
        #self.Conv1X1 = Conv(oup * 3, oup, 1, act=False)
    def forward(self, x):
        '''
        x_g: global features
        x_l: local features
        ...
        #global_feat = torch.cat([global_feat] + [global_feat1X3] + [global_feat3X1], 1)
        #global_feat = self.Conv1X1(global_feat)
        #out1 = local_feat * sig_act + global_feat
