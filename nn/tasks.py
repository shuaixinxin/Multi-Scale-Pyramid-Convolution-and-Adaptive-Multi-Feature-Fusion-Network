...
from ultralytics.nn.MS import MSBlock, C3_MSBlock
from ultralytics.nn.gold_yolo import Low_IFM, Low_LAF, Inject, SimConv, Low_FAM, High_LAF, High_FAM, Split, RepBlock, \
    High_IFM
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    Concat, Conv, ConvTranspose, Detect, DWConv, DWConvTranspose2d, Ensemble, Focus,
                                    GhostBottleneck, GhostConv, Pose, Segment, C2f_DCN, WeightedPermuteMLP, CBAM,
                                    CBAMC3, DCNv2, CARAFE, MPAMF, Conv1, MP, MPAMFpro,
                                    C2fGhostBottleneck, C2fMPAM, SELayer)
from ultralytics.nn.phd_benel_Glod_YOLO_DSC import SimFusion_3in, IFM, IFM_MPAMFpro, InjectionMultiSum_Auto_pool,InjectionMultiSum_Auto_poolx, SimFusion_4in, \
    PyramidPoolAgg, AdvPoolFusion, TopBasicLayer, DSC
from ultralytics.yolo.utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, colorstr, emojis, yaml_load
from ultralytics.yolo.utils.checks import check_requirements, check_suffix, check_yaml
from ultralytics.yolo.utils.plotting import feature_visualization
from ultralytics.yolo.utils.torch_utils import (fuse_conv_and_bn, fuse_deconv_and_bn, initialize_weights,
                                                intersect_dicts, make_divisible, model_info, scale_img, time_sync)
from ultralytics.nn.AFPN import ASFF_2,ASFF_3
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    ##
        ...
        if m in (Classify, Conv, DSC, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, C2f_DCN, C2fGhostBottleneck, C2fMPAM, DCNv2, SimConv):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, C2f_DCN):
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is MSBlock:
            c1, c2 = ch[f], args[0]
            args = [c1, c2, *args[1:]]
        elif m is C3_MSBlock:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            ...
            c2 = args[0]
            args = [c1, *args]
        elif m is InjectionMultiSum_Auto_poolx:
            c1 = ch[f[0]]
            c2 = args[0]
            args = [c1, *args]
